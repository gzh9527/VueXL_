  ...
  <div class="wrapper">
    ...
    <div class="card">
      ...
      <div class="cardText">
        ...
        <button @click="openVip();">开通权益</button>
      </div>
    </div>
    <h5>往期会员故事</h5>
    <div class="huzhuCase">
    ...
    <div class="footerBtn">
      <span @click="LiknMyVip()">我的权益</span>
      <span @click="openMysinahz()">我的互助</span>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    //获取publicid
    this.channel = this.$route.query.channel || '10003';
    this.$get(
      Api.getPublicList,
      {
        page: this.page,
      ...
      this.$get(Api.getPublicData, { public_id: this.publicId }, true).then(
        (res) => {
          console.log(res, ".....res");
  ...
  methods: {
    //了解更多
    learnMore() {
      this.actionCount("learnMore_click");
      // /weibo/public-member?id=36877de7b1e5494e8309fd107b47ecd3&planCode=XL001
      this.$router.push(
        `/weibo/public-member?id=${this.publicId}&planCode=XL001&channel=${this.channel}`
      );
    },
    //开通权益
    openVip() {
      this.actionCount("openVip_click");
      //判断用户是否加入互助
      this.$get(Api.getMyPolicy, {}).then((ret) => {
        if (ret.code === 0) {
          console.log(ret.data.plan_list_state[this.planCode], ".111111111111");
          if (
            ret.data.plan_list_state &&
            ret.data.plan_list_state[this.planCode] == "not_join"
          ) {
            this.$router.replace("/weibo/insure-plan?channel=" + this.channel);
          } else if ((ret.data.plan_list_state[this.planCode] = "not_active")) {
            //去开通免密支付
            this.$router.replace(
              `/weibo/insure-active?planCode=XL001&channel=${this.channel}`
            );
          } else {
            this.$router.push(
              "/weibo/weibo/share-list?planCode=" +
                "XL001" +
                "&channel=" +
                this.channel
            );
    ...
    //我的互助权益
    LiknMyVip() {
      this.actionCount("LiknMyVip_click");
      this.actionCount("openVip_click");
      //判断用户是否加入互助
      this.$get(Api.getMyPolicy, {}).then((ret) => {
        if (ret.code === 0) {
          ...
          if (
            ...
            //去加入
            this.$router.replace("/weibo/insure-join?channel=" + this.channel);
          } else if (ret.data.plan_list_state[this.planCode] == "not_active") {
            //去开通免密支付
            this.$router.replace(
              `/weibo/insure-active?planCode=XL001&channel=${this.channel}`
    ...
    //我的互助
    openMysinahz() {
      this.actionCount("openMysinahz_click");
      this.actionCount("openVip_click");
      //判断用户是否加入互助
      this.$get(Api.getMyPolicy, {}).then((ret) => {
        if (ret.code === 0) {
    ...
    goBack() {
      this.$router.replace({ path: "/weibo/insure-plan",query:{channel:this.channel}});
