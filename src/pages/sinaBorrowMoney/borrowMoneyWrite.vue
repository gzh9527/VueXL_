<template>
  <div class="sina_page">
    ...
    <div class="sina_page_bot">
      <div class="bot_btns">
        <div
          class="bot_btn primary flex1"
          style="background: linear-gradient(270deg, #ff4469 0%, #ff6546 100%)"
          @click="addJoin()"
        >
          立即完善信息
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      imgsList: [
        {
          url:
            "http://sinahuzhu.oss-cn-beijing.aliyuncs.com/images/sina_publicity_temp/smrz.png",
        },
        {
          url:
            "http://sinahuzhu.oss-cn-beijing.aliyuncs.com/images/sina_publicity_temp/openPay.png",
        },
        {
          url:
            "http://sinahuzhu.oss-cn-beijing.aliyuncs.com/images/sina_publicity_temp/pic3.png",
        },
        {
          url:
  ...
  methods: {
    ...
    addJoin() {
      ...
      let carId = this.userInfo.idNo.substring(
        this.userInfo.idNo.length - 2,
        this.userInfo.idNo.length - 1
      );
      //只添加本人
      ...
      let params = {
        ...
        channel: channel,
        amount: 0,
        amnt_data: [{ XL001: 50000 }],
        deduct_data: [{ XL001: 2 }, { XL002: 2 }],
      };
      ...
      if (carId % 2 == 0) {
        params.deduct_data = [
          { XL001: { state: 2, valid_day: 180 } },
          { XL002: { state: 2, valid_day: 180 } },
        ];
      } else {
        //男性送3元红包
        this.$post(Api.joinTicket, {
      ...
      this.$post(Api.borrowJoinPlan, params).then((ret) => {
        if (ret.code === 0) {
          if (this.isChecked) {
...
.sina_page .sina_hasbg.write_bg {
  background: url(http://sinahuzhu.oss-cn-beijing.aliyuncs.com/images/sina_publicity_temp/haderbg.png);
  background-repeat: no-repeat;
  background-size: 100% auto;
}
</style>
