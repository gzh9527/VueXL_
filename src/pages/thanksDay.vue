  ...
  <div class="sina_page" :class="{ haspagebot: ishasbot }" ref="sinaPage">
    ...
    <template v-if="channelType && channelType == 'redChannel'">
      <div
        ...
        :class="{
          channel_nogive: channelType != 'valentChannel',
          channel_valent: channelType == 'valentChannel',
        }"
      >
        <div class="sina_join_cont">
          <div class="share_desc" v-if="channelType == 'valentChannel'">
            <span v-if="shareInfo.name"
              >你的{{ shareInfo.relation
              }}{{ shareInfo.name }}送你互助红包</span
            >
          </div>
          <div class="join_btn">
            <input
...
export default {
  ...
  methods: {
    ...
    checkChannel() {
      ...
      console.log(channel, ".......channelchannelchannel");
      if (!channel) {
        window.localStorage.setItem("channel", 10000);
      } else {
  ...
  .sina_hasbg {
    ...
    &.maxbg {
      ...
      &.share_bg {
        ...
        background: url("../assets/images/jfLogo.png");
      }
      &.give_bg {
        //background-image: url("http://bihu-oss.oss-cn-beijing.aliyuncs.com/images/sina_weibo/DBFormGiveBg.png");
        background: url("http://sinahuzhu.oss-cn-beijing.aliyuncs.com/images/sina_publicity_temp/jfLogo.png")
          100% 100%;
