<template>
  <div class="sina_page" :class="{ haspagebot: ishasbot }" ref="sinaPage">
    <template  v-if="channelType&&channelType=='redChannel'"  >
      <div class="sina_hasbg " :class="{channel_nogive:channelType!='valentChannel',channel_valent:channelType=='valentChannel' }">
        <div class="sina_join_cont">
          <div class="share_desc" v-if="channelType=='valentChannel'">
            <span v-if="shareInfo.name">您的{{shareInfo.relation}}{{shareInfo.name}}向你告白，并送你：</span>

          </div>
          <div class="redPackBg" :class="{redpack:channelType!='valentChannel', valent:channelType=='valentChannel'}">
            <div class="join_btn">
              <input type="tel" autocomplete="off" maxlength="11" class="input_css"
                     v-model.trim="mobile" placeholder="请输入手机号"/>
              <button class="sina_btn primary radius max" @click="addPlan()">立即领取</button>
              <div class="check_box primary bg3" :class="{checked:isChecked}" @click.stop="isChecked=!isChecked">
                关注@新浪互助，参与互助事件、章程审议等
              </div>
            </div>
          </div>
          <img class="intro_bg" src="http://bihu-oss.oss-cn-beijing.aliyuncs.com/images/sina_weibo/DBWriteFormIntro3_2x.png" />
        </div>
      </div>
    </template>

    <div class="sina_hasbg maxbg " :class="{give_bg:channelType=='giveChannel',share_bg:channelType=='valentChannel'}" v-if="channelType&&channelType!='redChannel'">
      <div class="sina_join_cont" >
        <div class="share_desc" v-if="channelType=='valentChannel'">
          <span v-if="shareInfo.name">你的{{shareInfo.relation}}{{shareInfo.name}}送你互助红包</span>
        </div>
        <div class="join_btn">
          <input type="tel" autocomplete="off" maxlength="11" class="input_css"
                 v-model.trim="mobile" placeholder="请输入手机号"/>
          <button class="sina_btn primary radius max" @click="addPlan()">立即领取</button>
          <div class="check_box bg1" :class="{checked:isChecked}" @click.stop="isChecked=!isChecked">
            关注@新浪互助，参与互助事件、章程审议等
          </div>
        </div>
    ...
    <div class="sina_img_cells">
      ...
      <div class="sina_img_cell hasiconBg case">
        <img v-lazy="'https://imgs.bihubao.com/Public/images/insureAdView/images/DBintroImg6_2x.png'"
        ...
        <div class="QA-cells">
          <template v-for="(item,idx) in introQAlist">
            <div class="QA-cell" :key="idx">
              <div class="qa-title cell-arrow" :ref="'qaTitle'+idx" :class="{open:item.isShowAn, hot:item.isHot}" @click="showIntoAn(idx)">{{item.question}}</div>
    ...
    <transition name="translate-bottom">
      <div class="sina_join_bot " v-if="ishasbot">
        <button class="sina_btn primary radius max" @click="addPlan()">{{channelType=='redChannel' || channelType == 'defChannel' ?'领取3元红包':'立即领取'}}</button>
        <div class="check_box primary bg3" :class="{checked:isChecked}" @click.stop="isChecked=!isChecked">
          关注@新浪互助，参与互助事件、章程审议等
        </div>
      </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        imgsList:[
          {url: 'https://imgs.bihubao.com/Public/images/insureAdView/images/DBintroImg1_2x.png'},
          {url: 'https://imgs.bihubao.com/Public/images/insureAdView/images/DBintroImg2_2x.png'},
        ],
        adInfo:{
          img_url:'https://imgs.bihubao.com/Public/images/insureAdView/insureAdRedpack1.png',
          btn_url:'https://imgs.bihubao.com/Public/images/insureAdView/insureRedpackBtn.gif',
    ...
    methods: {
      ...
      getPlanStatus() {
        this.$get(Api.getMyPolicy).then(ret => {
          if (ret.code === 0) {
            if (ret.data.plan_list_state && (ret.data.plan_list_state['XL001'] != 'not_join' || ret.data.plan_list_state['XL002'] != 'not_join')) {
      ...
      addPlan() {
        ...
        if(channel&&channel==10021){
          window.location.href = 'sinaweibo://browser?url=https%3A%2F%2Fsina.bihubao.com%2Fweibo%2Finsure-write%3Fchannel%3D10021'
        }else{
          this.$router.push(`/weibo/insure-write?hasChecked=${hasChecked}&phone=${this.mobile}&name=${this.shareInfo.name || ''}`);
        }
  ...
  .sina_page {
    ...
    .sina_hasbg {
      ...
      .sina_join_cont {
        ...
        .max{
          height: 100px;
          background:linear-gradient(90deg,rgba(255,233,210,1) 0%,rgba(255,227,170,1) 100%);
          box-shadow:none;
          color: #FF5714;
          border-radius:50px;
          font-size: 40px;
