        ...
        <div class="sina_radius_bg ">
          <div class="detail_box">
            <div class="detail_info">
              <div class="hasborTitle">{{planInfo&&planInfo.insured.name}} |
                {{planInfo&&planInfo.plan.plan_name || '意外医疗计划 '}}
              </div>
              <div class="info_cont">
                <div class="days">已保障 {{planInfo&&planInfo.plan.join_day}} 天<span
                  v-if="planInfo&&planInfo.plan.valid_state!='1' &&planInfo.plan.plan_code !='XL003' " @click="showInfoAlert">(未过{{this.planInfo.plan.plan_code=='XL001'?'观察期':'等待期'}})<i
                  class="info_icon"></i></span></div>
                <div class="label">计划编号</div>
              </div>
            </div>
<!--            <div class="state_icon"></div>-->
          </div>
          <div class="plan_no">{{planInfo&&planInfo.plan.son_policy_no}}</div>
        </div>
        <div class="sina_cont sina_btns flex">
          <div class="sina_btn primary radius line medium" v-if="planInfo&&planInfo.insured.notHaveCode =='XL001'"
               @click.stop="addPlanAlert(planInfo.insured)">0元升级保障</div>
...
<!--          <div class="sina_btn primary radius  medium" v-if="planInfo&&planInfo.plan.plan_code=='XL001'"-->
<!--               @click="updateAmnt">互助金调额</div>-->
          <div class="sina_btn primary radius  medium" v-if="planInfo&&planInfo.plan.plan_code=='XL001'"
               @click="updateTypeName">调整受助类型</div>
          <div  class="sina_btn primary radius line medium" @click="addReport">申领互助金</div>
    ...
    <div class="plan_hasback">
      ...
      <div class="user_cells sina_cont">
        <div class="user_cell flex">
          <div class="label">计划编号</div>
          <div class="info flex1">{{planInfo&&planInfo.plan.son_policy_no}}</div>
        </div>
        <div class="user_cell flex">
          <div class="label">加入时间</div>
          <div class="info flex1">{{planInfo&&planInfo.plan.join_date}}</div>
        </div>
        <div class="user_cell flex">
          <div class="label">生效时间</div>
          <div class="info flex1">{{planInfo&&(planInfo.plan.obsdate || planInfo.plan.valid_date)}}</div>
        </div>
        <div class="user_cell flex">
        ...
        <div class="user_cell flex" v-if="planInfo&&planInfo.plan.plan_code=='XL001'">
          <div class="label">受助类型</div>
          <div class="info flex1">{{planInfo&&planInfo.plan.duty_type_name || '报销型'}}</div>
        </div>
        <table class="term_table" v-if="planInfo&&planInfo.plan.plan_code=='XL001'">
          ...
          <tr>
            <td colspan="2">身故</td>
            <td class="twoline">互助额度的1/12或既往支出的所分摊互助金额，取两者之中最大值。</td>
          </tr>
        </table>
        <table class="term_table" v-if="planInfo&&planInfo.plan.plan_code=='XL002'">
          <tr>
        ...
        <img src="./images/FYdetailTable_2x.png" v-if="planInfo&&planInfo.plan.plan_code=='XL003'">
      </div>
      <div class="join_clause" v-if="planInfo">
        <router-link v-if=" planInfo.plan.plan_code!='XL003'" :to=" planInfo.plan.plan_code=='XL001' ?'/weibo/clause?type=0&cid=0':'/weibo/clause?type=0&cid=5'" class="col_a">《健康告知》</router-link>
        <router-link to="/weibo/clause?type=0&cid=1" class="col_a">《会员公约》</router-link>
        <router-link :to=" planInfo.plan.plan_code=='XL001' ?'/weibo/clause?type=0&cid=2' : planInfo.plan.plan_code=='XL002' ? '/weibo/clause?type=0&cid=4' : '/weibo/clause?type=0&cid=6'" class="col_a">《计划章程》</router-link>
        <router-link to="/weibo/clause?type=0&cid=3" class="col_a">《授权代扣委托书》</router-link>
      </div>

    </div>
    <div class="sina_cont">
      <button class="sina_btn primary radius max"  @click="gotoCenter()">查看个人中心</button>
     <div class="plan_delbtn" @click="giveupPlanAlert">放弃保障</div>
    </div>
    ...
    <plan-type v-if="isShowPlanType"
               ...
               @closeTypeAlert="closeTypeAlert"></plan-type>
...
<script>
  ...
  export default {
    components: {
      myService,oneBtnJoin,clauseAlert,planType
    },
    mixins: [mixinActions],
    name: "planDetail",
    data() {
    ...
    methods: {
      ...
      showInfoAlert() {
        ...
        if(this.planInfo.plan.plan_code=='XL001'){
          ...
          title ='什么是等待期';
          alertDesc ='<p>自成员加入计划生效日零时起30日内（含30天）为等待期。</p>';
        }
        this.$sinaAlert({
          title: title,        //提醒文字
          onlyHasTitle: false,
      ...
      giveupPlanAlert() {
        if(this.planInfo.plan.plan_code == 'XL001'||this.planInfo.plan.plan_code == 'XL002'){
          this.actionCount('delBtn_click',()=>{
            this.$router.push(`/weibo/exit-paln?id=${this.id}&uuid=${this.planInfo.plan.policy_uuid}&planCode=${this.planInfo.plan.plan_code}`)
          });
        }else{
          this.actionCount('delBtn_click');
          let alertTitle ='';
          if(this.planInfo.plan.plan_code=='XL001'){
            alertTitle ='现在退出，最高30万重疾互助金将失效，再加入将重新确认健康要求，并重新计算180天观察期。本人放弃保障家人也将失去守护，你要离开吗';
          }else if(this.planInfo.plan.plan_code=='XL002'){
            alertTitle ='现在退出，最高5万意外医疗互助金将失效，再加入将重新确认健康要求，并重新计算30天等待期。你要离开吗';
      ...
      addReport(){
        if (this.planInfo.plan.valid_state == '1') {
          let weiboInfo = sessionStorage.getItem('weiboInfo') ? JSON.parse(sessionStorage.getItem('weiboInfo')) : null;
          let {hostname} = window.location;
          let url = hostname == 'sina.bihubao.com' ? 'https://tpa.bihubao.com' : 'http://tpatest.bihubao.com';
          if (weiboInfo) {
      ...
      updateAmnt(){
        this.$router.push('/weibo/amnt-edit?planCode=XL001&customerNo='+this.planInfo.insured.customerno)
      },
      closeTypeAlert(type){
