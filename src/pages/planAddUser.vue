  ...
  <div class="sina_page">
    <div class="sina_hasbg">
      ...
      <div class="sina_cont">
        <div class="who_cells">
          <div class="who_cell">
            <div class="who_btn " v-if="relationLimit&&relationLimit.spouseShow"
                 @click="relation=51"
                 :class="{active:relation==51}">
              <i class="who_icon spouse"></i>
              <i class="checkbox"></i>配偶
            </div>
            <div class="who_btn " v-if="relationLimit&&relationLimit.parentShow"
                 @click="relation=53"
            ...
            <div class="who_btn "
                 @click="relation=52" :class="{active:relation==52}">
        ...
        <div class="input_cells">
          ...
          <div class="sina_input_normal hasarrow right" @click="workTypeShow=true">
        ...
        <div class="join_intro">
          <div class="join_desc hasradius_bg" >
            <i class="checkbox" @click.stop="isCheckedFY=!isCheckedFY"><i v-if="isCheckedFY" class="checked"></i></i>
            <div class="plan_title">新冠病毒互助最高10万</div>
            <div class="plan_desc">不限年龄，医护人员最高30万</div>
            <div class="plan_clause">
              <a href="javascript:;" @click.stop="showAlert(1)" class="col_a">《会员公约》</a>
              <a href="javascript:;" @click.stop="showAlert(6)" class="col_a">《计划章程》</a>
            </div>
          </div>
          <div class="join_desc hasradius_bg" >
            <i class="checkbox" @click="isChecked=!isChecked"><i v-if="isChecked" class="checked"></i></i>
            <div class="plan_title">大病互助+意外互助最高35万</div>
            <div class="plan_desc">重疾最高30万+意外医疗最高5万</div>
            <div class="plan_clause">
              <a href="javascript:;" @click.stop="showAlert(1)" class="col_a">《会员公约》</a>
              <a href="javascript:;" @click.stop="showAlert(0)" class="col_a">《健康告知》</a>
              <a href="javascript:;" @click.stop="showAlert(2)" class="col_a">《计划章程》</a>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="sina_cont myAd">
        <my-ad id="weibo-insure-add-1"></my-ad>
        <div class="join_btn">
          <button class="sina_btn primary radius" @click="addUserEvent">0元加入</button>
        </div>
      </div>
    </div>
      ...
      <div class="add_suc_con">
        <div class="sina_radius_bg">
          <div class="add_suc_cont">
            <div class="add_suc_name">{{userInfo.userName}}
              <span class="icon" v-if="relation == 51">配偶</span>
              <span class="icon" v-if="relation == 53">父母</span>
              <span class="icon" v-if="relation == 52">子女</span></div>
            <div class="add_suc_txt">已加入成功</div>
            <div class="check_box bg" :class="{checked:sendchecked}" @click.stop="sendchecked=!sendchecked">
              发微博邀请更多好友共享互助
            </div>
          </div>
          <div class="add_bot_btns">
            <div class="add_bot_btn sina_btn radius default" @click="jumpUrl('plan')">返回个人中心</div>
            <div class="add_bot_btn sina_btn radius primary " @click="jumpUrl('adduser')">继续添加家人</div>
    ...
    methods: {
      getPlanStatus() {
        this.$get(Api.getMyPolicy, {}).then(ret => {
          if (ret.code === 0) {
            if (ret.data.plan_list_state && (ret.data.plan_list_state['XL001'] != 'activated'&& ret.data.plan_list_state['XL002'] != 'activated'  && ret.data.plan_list_state['XL003'] != 'activated')) {
      ...
      getLimit() {
        this.$get(Api.getRelationLimit, {}).then(ret => {
          if (ret.code == 0) {
            ...
            if ((res['spouse_count'] > 0 && res['parent_count'] < 4) || (res['spouse_count'] == 0 && res['parent_count'] < 2)) {
      ...
      addUserEvent() {
        ...
        this.$post(Api.addPolicy, {
          ...
          plan_code:code,
          channel: this.$route.query.channel || localStorage.getItem('channel') || '10000'
        }).then(ret => {
          if (ret.code === 0) {
